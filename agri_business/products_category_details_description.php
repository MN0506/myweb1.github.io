...
<li class="has-child"><a>Product Category Details</a></li>
...
$prod_det_id=base64_decode($_REQUEST["pd_id"]);
...
$sql=$conn->prepare("SELECT * FROM product_details pd,product_category pc,stock_details s WHERE pd.pc_id=pc.pc_id AND  pd.pd_id=s.pd_id AND pd.pd_id=? AND pc.pc_id=?");
$sql->bind_param("ii",$prod_det_id,$prod_cat_id);
$sql->execute();
...
<p class="product-title" style="font-size:18px;"><?php echo $row2["pd_name"];?></p>
